...
const getDistanceFromLatLonInKm = (lat2, lon2) => {
  ...
  var dLat = deg2rad(lat2 - 51.5144636); // deg2rad below
  var dLon = deg2rad(lon2 - -0.142571);
  var a =
    Math.sin(dLat / 2) * Math.sin(dLat / 2) +
    Math.cos(deg2rad(51.5144636)) *
...
const sortFunction = (a, b) => {
  const nameA = a.organization.toUpperCase(); // ignore upper and lowercase
  const nameB = b.organization.toUpperCase(); // ignore upper and lowercase
  if (nameA < nameB) {
    return -1;
  }
  if (nameA > nameB) {
...
// Getting By Distance
router.get("/:distance", getPartners, (req, res) => {
  res.json(res.partners.sort(sortFunction));
});
...
async function getPartners(req, res, next) {
  ...
  try {
    ...
    // filter by distance
    if (partners == null) {
      return res.status(404).json({ message: "Cannot find partners" });
    }
    partners = partners.filter((partner) => {
      let coordinates = partner.offices[0].coordinates.split(",");
      let distanceToFilterDataBy = req.params.distance;
      let distanceFromLondonOffice = getDistanceFromLatLonInKm(coordinates[0], coordinates[1]);
      return distanceToFilterDataBy > distanceFromLondonOffice;
    });
